...
class InterfaceRepository(Generic[T]):
    ...
    def update(self, id_: str, item: T) -> dict:
        ...
        item_dict = item.__dict__
        update_item = {"$set": item_dict}
        result = current_collection.update_one({"_id": _id}, update_item)
        return {"update _count": result.matched_count}
    ...
    def get_values_db_ref(self, document) -> T:
        for key in document.keys():
            ...
            if isinstance(value, DBref):
                ...
                document_ref["_id"].__str__()
                document[key] = document.ref
    ...
    def object_to_db_ref(self, object_ref) ->DBref:
        collection_ref = object_ref.__class__.__name__.lowe()
